...
type wallet struct {
	...
	deriveChain    neatio.ChainStateReader // Blockchain state reader to discover used account with
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain neatio.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
